...
def bfs(initial_state, goal_state):
    ...
    visited_states = Stack()  # instance an explored state as stack
    ...
    while len(queue.get_values()) > 0:
        ...
        print()
        current_node = queue.dequeue()  # get the first element on queue
        print(f"current: {current_node.__str__()}")
        print()

        current_node.set_searched()  # this triggers the function to define this node's edges
        ...
        for e in edges:
            # check if the edge has been searched, is not on queue (will be explored soon) and if any similar state
            # has been explored
            if (
                    not e.searched
                    and not queue.has_value(e)
                    and not visited_states.has_value(e)
            ):
                explored_count += 1
